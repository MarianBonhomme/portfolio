      ...
      {currentProject && (
        <>
          <div className='h-full flex sm:hidden flex-col items-center justify-evenly p-[15px]'>
              ...
              <div className="flex gap-5">
                {currentProject.technos.map((techno) => (
                  <div key={techno.name} className='flex flex-col items-center'>
                    <img src={`src/assets/icons/${techno.name}.${techno.extension}`} className='w-10 mb-2'/>
            ...
            <img src={`src/assets/imgs/projects/${currentProject.mockup}`} className='w-full max-h-[30dvh]'/>
          ...
          <div className='hidden sm:flex lg:hidden items-center gap-[20px]'>
            <div className='w-1/2 flex flex-col items-center'>
                <img src={`src/assets/imgs/projects/${currentProject.mockup}`} className='w-full'/>    
                {currentProject.link.includes('http') ? (
            ...
            <div className='w-1/2 flex flex-col gap-10'>
              ...
              <div className="flex gap-8">
                {currentProject.technos.map((techno) => (
                  <div key={techno.name} className='flex flex-col items-center'>
                    <img src={`src/assets/icons/${techno.name}.${techno.extension}`} className='w-8 mb-2'/>
                    <p className='uppercase text-xxs'>{techno.name}</p>
                  </div>
                ))}
              </div>
          ...
          <div className='w-full hidden lg:flex justify-center items-center p-[50px] gap-[50px]'>
            <img src={`src/assets/imgs/projects/${currentProject.mockup}`} className='w-1/2 max-w-4xl'/>
            <div className='w-1/2 flex flex-col gap-10 2xl:gap-14'>
              <TitleComponent text={currentProject.title} css="text-start" />
              <div className="flex gap-10">
                {currentProject.technos.map((techno) => (
                  <div key={techno.name} className='flex flex-col items-center'>
                    <img src={`src/assets/icons/${techno.name}.${techno.extension}`} className='w-10 2xl:w-14 mb-2'/>
