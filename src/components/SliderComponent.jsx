      ...
      >
        {projects.map((project, index) => (
          <SwiperSlide key={index}>
            <img src={`/assets/images/${project.img}`} className="absolute top-1/2 -translate-y-[100%] sm:-translate-y-[75%]"/>
        ...
        <div className={`arrow-prev fixed bottom-[15%] left-1/4 z-40 text-5xl 2xl:text-7xl font-black transition-all duration-500 
          ${currentSection === 2 ? 'cursor-pointer' : 'opacity-0'}
          ${!prevButtonVisible && 'opacity-0 cursor-default'}`} >←</div>
        <div className={`arrow-next fixed bottom-[15%] right-1/4 z-40 text-5xl 2xl:text-7xl font-black transition-all duration-500 
          ${currentSection === 2 ? 'cursor-pointer' : 'opacity-0'}
